...
class ProfileScreen extends StatefulWidget {
  const ProfileScreen({ Key? key, required this.onLogOut, required this.customerInfo }) : super(key: key);
      ...
      body: Stack(children: [
        ...
        Positioned(top: 120, left: 30, right: 30, child: Column(
          ...
          children: [
            ...
            RegularTextField(controller: usernameController,    labelText: "Tên người dùng", text: widget.customerInfo["username"]),
            RegularTextField(controller: phonenumberController, labelText: "Số điện thoại", text: widget.customerInfo["phonenumber"]),
            const SizedBox(height: 15),
            BigButton(width: 240, fontSize: 18, label: "Đổi mật khẩu", onPressed: () {}),
            const SizedBox(height: 30),
            Container(height: 2, color: Colors.yellow.shade200),
            const SizedBox(height: 30),
            BigButton(width: 240, label: "Thay đổi thông tin", onPressed: () => Navigator.pop(context)),
          ],
        )),
        ...
        Positioned(bottom: 30, left: 0, right: 0, child: Center(
          child: BigButton(
            ...
            label: "Đăng xuất",
            onPressed: () {
              if (mounted) { setState(() => widget.onLogOut() ); }
              else { throw "[MOUNTED ERROR AT PROFILE SCREEN]"; }
            }
          ),
        ))
