...
class AccountReader {
  ...
  Future< Map<String, dynamic> > logOut() async {
    ...
    final response = await http.post(Uri.parse(Auth.logout), headers: {
        ...
        "authentication": await TokenSaver().loadRefreshToken()
      });
  ...
  Future< String > getNewTokens() async {
    ...
    final response = await http.post(Uri.parse(Auth.refresh),
                                    headers: { "Content-Type": "application/json; charset=UTF-8", "authentication": await TokenSaver().loadRefreshToken() });
